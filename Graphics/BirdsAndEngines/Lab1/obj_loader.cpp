...
IndModel OBJModel::ToIndexedModel()
{
    IndModel result;
    IndModel normalModel;
    ...
    for(unsigned int i = 0; i < numIndices; i++)
    {
        ...
        if(it == normalModelIndexMap.end())
        {
            ...
            normalModel.tC.push_back(currentTexCoord);
            normalModel.n.push_back(currentNormal);
        }
        else
        ...
        if(previousVertexLocation == (unsigned int)-1)
        {
            ...
            result.tC.push_back(currentTexCoord);
            result.n.push_back(currentNormal);
        }
        else
            resultModelIndex = previousVertexLocation;
        
        normalModel.ind.push_back(normalModelIndex);
        result.ind.push_back(resultModelIndex);
    ...
    if(!hasNormals)
    {
        ...
        for(unsigned int i = 0; i < result.pos.size(); i++)
            result.n[i] = normalModel.n[indexMap[i]];
    }
    
    return result;
};

unsigned int OBJModel::FindLastVertexIndex(const std::vector<OBJIndex*>& indexLookup, const OBJIndex* currentIndex, const IndModel& result)
{
                    ...
                    && (!hasNormals || possibleIndex->normalIndex == currentIndex->normalIndex))
                {
                    ...
                    for(unsigned int j = 0; j < result.pos.size(); j++)
                    {
                        if(currentPosition == result.pos[j] 
                            && ((!hasUVs || currentTexCoord == result.tC[j])
                            && (!hasNormals || currentNormal == result.n[j])))
                        {
                            return j;
                        }
                    }
